...
dbHeader <- dashboardHeader(
  title = div( "menv",style = "color: white; margin-left: -10px; font-size: 22px; ffont-family: 'Amita', cursive"),
  titleWidth = 150
)
...
server <- function(input, output, session) {
  ...
  getsom <- reactive({
    ...
    {
      attr(saved_data$df[[input$data_som]],"som")[[input$som_models]]
    } else {som.reactive()}
  })
...
output$textbreak<-renderText("This action creates a single binary column per factor level, with 1 indicating the class of that particular observation")
  ...
  datahand_out<-reactive(
    modalDialog(
      switch(datahand$df,
             'Include binary columns in the Data-Attribute'=classmat_out(),
             'Save changes'="Datalist successfully saved",
             'Save newsom in'="Som model successfully saved",
             'Save Clusters'="Clusters successfully saved"
      ),
      size="m",
      easyClose = T
    )
  ...
  output$data_create<-renderUI({
    ...
    if(datahand$df=="Save newsom in"){textInput("model_newname", NULL,paste("Som", length(attr(getdata_som(),"som"))+1))} else
      if(datahand$df=="Save changes"|datahand$df=="Include binary columns in the Data-Attribute"){
      textInput("data_newname", NULL,paste("Datalist", length(saved_data$df)+1))} else
      ...
      fluidRow(
        splitLayout(
          ...
          column(12,
                 ...
                 textInput("data_name", NULL, value=paste("Datalist",length(saved_data$df)+1))
                 ,
  ...
  observeEvent(input$tools_savesom,{
    if(input$tools_savesom %% 2){
      ...
      datahand3$df<-p("Both 'create' and 'overwrite' options add the ",p(tipify(actionLink("preview_bmu",'resulting BMU'),"click to preview")," to the 'Factor-Attribute'"),p("of the corresponding Datalist."))
      showModal(
        datahand_modal()
      )
    }
  ...
  output$preview<-renderUI({
    ...
    } else if(datahand$df=="Save changes")
    {
      column(12,
             style="overflow-x: scroll;height:300px;overflow-y: scroll",
             column(12,strong("Preview of the Data-Attribute")),
    ...
    }    else if(datahand$df=="Create data list from aggregation results")
    {
      column(12,
             style="overflow-x: scroll;height:300px;overflow-y: scroll",
             column(12,strong("Preview of the aggregatted results")),
             column(12,renderPrint(dim(attr(aggreg(),"coords")))),
             column(12,renderTable(
  ...
  output$downloadData <- downloadHandler(
    ...
    content = function(name1) {
      fs <- c()
      fs <-
        c('meta',"code_to_run.R","app.R","menvi.R"
  ...
  labels <- reactive({
    ...
    if (input$up_or_ex == 'upload') {
      ...
      labels <-
        data.frame(fread("meta/example_factors_araca.csv", stringsAsFactors = T))
  ...
  coords<-reactive({
    ...
    if (input$up_or_ex == 'use example data') {
      ...
      coords <-data.frame(fread("meta/example_coords_araca.csv"))} else {
  ...
  aggreg<-reactive({
    data<-getdata_upload()
    factors<-attr(data,"factors")[rownames(data),]
    ...
    if(ncol(factors[input$fac_descs])==1){
      ...
      if(!is.null(coords)){
      coords<-do.call("cbind",lapply(data,function(x, fac) tapply(x,factors[,input$fac_descs] ,mean)))
      coords<-coords[,which(unlist(lapply(coords,is.numeric)))]
      rownames(coords)<-rownames(df)
      }

      facs<-data.frame(rownames(df))
  ...
  dataraw <- reactive({
    if (input$up_or_ex == 'use example data') {
      data <-
        data.frame(fread("meta/example_nema_araca.csv", stringsAsFactors = T))
  ...
  getdatalist<-reactive({
    ...
    if(input$up_or_ex == 'use example data'){
      attr(data,"filename")<-'example_nema_araca.csv'
  ...
  finesom <- reactive({
    ...
    fluidRow(
      ...
      column(6, selectInput(
        "mode", "mode", choices = c("online", "batch", "pbatch")
      )),
      column(
        6,
        numericInput(
  ...
  output$showgrid <- renderPlot({
    ...
    m <-
      supersom(
        as.matrix(data),
        grid = kohonen::somgrid(input$xdim, input$ydim, topo = input$topo),
        rlen = 5
      )
    plot(
      m,
  ...
  #rfmodal
  {
    rfmodal <- function() {
      modalDialog(
        textrf(),
        title = "Random Forest from caret",
        footer = modalButton("close"),
        size = "m",
        easyClose = TRUE
      )
  ...
  output$dt_panel<-renderUI({
    column(12,style="background: white; margin: 20 20 20 20",
           ...
           column(12,renderPrint(head(get_supdt())[,1:6])),
  ...
  output$ptree_control <- renderUI({
    column(
      ...
      strong("ctree control", actionLink(
      ...
      splitLayout(
        ...
        numericInput("mincriterion", "mincriterion", 0.95),
        numericInput("minsplit", "minsplit", 20, step = 1),
        numericInput("minbucket", "minbucket", 7, step = 1),
        numericInput("nresample", "nresample", 999, step = 1)
      ),
      splitLayout(
        numericInput("maxsurrogate", "maxsurrogate", 0, step = 1),
        numericInput("mtry", "mtry", 0 , step = 1),
        numericInput("maxdepth", "maxdepth", 0 , step = 1),
        strong("Palette", style="color: #0D47A1"),
